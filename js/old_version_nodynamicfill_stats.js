	...
	d3.csv("players_full.csv", function(error, data) {
...
var pos_pos2 = ["GKP GKP", "GKP DEF", "DEF GKP"]
var pos_pos3 = ["DEF MID", "MID DEF", "MID GKP", "GKP MID"]
var pos_pos4 = ["GKP FWD", "DEF FWD", "FWD GKP", "FWD DEF"]
...
function load_stats (features,featuresx) {
	var elements_stat = ""
...
function update (data,chart_type) {
    ...
    var info_fitness_p1 = d3.select("#p1-fitness")
		.on('mouseover.tooltip', function(d) {
		    tooltip.transition()
		      .duration(300)
		      .style("opacity", 1);
	...
	var info_fitness_p2 = d3.select("#p2-fitness")
		.on('mouseover.tooltip', function(d) {
		    tooltip.transition()
		      .duration(300)
		      .style("opacity", 1);
    ...
    for (var i=0; i<features.length; i++) {
    	...
    	total_p1.push({'axis':`${features[i]}`, 'value':Nan_rep(eval(tot_varsp1[i])/(eval(tot_varsp1[i])+eval(tot_varsp2[i]))), "actual": `<strong>${$('#player1 option:selected').text().split(" ")[0]}</strong> <br><strong>${features[i]}:</strong> ${(d3.format(".2f"))(eval(tot_varsp1[i]))}`,"player":0}) 
    	total_p2.push({'axis':`${features[i]}`, 'value':Nan_rep(eval(tot_varsp2[i])/(eval(tot_varsp1[i])+eval(tot_varsp2[i]))), "actual":`<strong>${$('#player2 option:selected').text().split(" ")[0]}</strong> <br><strong>${features[i]}:</strong> ${(d3.format(".2f"))(eval(tot_varsp2[i]))}`,"player":1})
    };
    ...
    for (var i=0; i<features.length; i++) {
    	...
    	total_p1_pgs.push({'axis':`${features[i]}`, 'value':Nan_rep((eval(tot_varsp1[i])/p1_pgs.length)/(eval(tot_varsp1[i])/p1_pgs.length+eval(tot_varsp2[i])/p2_pgs.length)), "actual":`<strong>${$('#player1 option:selected').text().split(" ")[0]}</strong> <br><strong>${features[i]} per game started:</strong> ${(d3.format(".2f"))(eval(tot_varsp1[i])/p1_pgs.length)}`,"player":0})
    	total_p2_pgs.push({'axis':`${features[i]}`, 'value':Nan_rep((eval(tot_varsp2[i])/p2_pgs.length)/(eval(tot_varsp1[i])/p1_pgs.length+eval(tot_varsp2[i])/p2_pgs.length)), "actual":`<strong>${$('#player2 option:selected').text().split(" ")[0]}</strong> <br><strong>${features[i]} per game started:</strong> ${(d3.format(".2f"))(eval(tot_varsp2[i])/p2_pgs.length)}`,"player":1})
    };
    ...
    for (var i=0; i<features.length; i++) {
    	...
    	// window["p1Tot" + features[i]] = filtered_p1.reduce((total, obj) => obj[features[i]] + total,0);
    	// window["p2Tot" + features[i]] = filtered_p2.reduce((total, obj) => obj[features[i]] + total,0);
    	total_p1_90.push({'axis':`${features[i]}`, 'value':Nan_rep((eval(tot_varsp1[i])/90)/(eval(tot_varsp1[i])/90+eval(tot_varsp2[i])/90)), "actual":`<strong>${$('#player1 option:selected').text().split(" ")[0]}</strong> <br><strong>${features[i]} per 90min:</strong> ${(d3.format(".2f"))(eval(tot_varsp1[i])/90)}`,"player":0})
    	total_p2_90.push({'axis':`${features[i]}`, 'value':Nan_rep((eval(tot_varsp2[i])/90)/(eval(tot_varsp1[i])/90+eval(tot_varsp2[i])/90)), "actual":`<strong>${$('#player2 option:selected').text().split(" ")[0]}</strong> <br><strong>${features[i]} per 90min:</strong> ${(d3.format(".2f"))(eval(tot_varsp2[i])/90)}`,"player":1})
    };
 ...
 //    	var features2 = ["total_points", "bonus", "goals_conceded", "saves", "penalties_saved", "minutes", "own_goals","clean_sheets"]
    ...
    for (let i=0; i<total_p1.length;i++){
    	diverging_total.push({"rank":i,"name":total_p1[i].axis,"difference":total_p2[i].value - total_p1[i].value,"actual":`<br><strong>${$('#player1 option:selected').text().split(" ")[0]}:</strong> ${(d3.format(".2f"))(total_p1[i].actual)} <br> <strong>${$('#player2 option:selected').text().split(" ")[0]}:</strong> ${(d3.format(".2f"))(total_p2[i].actual)}`})
    }
    ...
    var diverging_pgs = []
    for (let i=0; i<total_p1.length;i++){
    	diverging_pgs.push({"rank":i,"name":total_p1_pgs[i].axis,"difference":total_p2_pgs[i].value - total_p1_pgs[i].value,"actual":`<br><strong>${$('#player1 option:selected').text().split(" ")[0]}:</strong> ${(d3.format(".2f"))(total_p1_pgs[i].actual)} <br> <strong>${$('#player2 option:selected').text().split(" ")[0]}:</strong> ${(d3.format(".2f"))(total_p2_pgs[i].actual)}`})
    }
    ...
    var diverging_90 = []
    for (let i=0; i<total_p1.length;i++){
    	diverging_90.push({"rank":i,"name":total_p1_90[i].axis,"difference":total_p2_90[i].value - total_p1_90[i].value,"actual":`<br><strong>${$('#player1 option:selected').text().split(" ")[0]}:</strong> ${(d3.format(".2f"))(total_p1_90[i].actual)} <br> <strong>${$('#player2 option:selected').text().split(" ")[0]}:</strong> ${(d3.format(".2f"))(total_p2_90[i].actual)}`})
    }
	...
	d3.csv("fixtures.csv", function(error, fix) {
		if (error) throw error;
		...
		d3.csv("teams_full.csv", function(error, teams) {
			if (error) throw error;
    ...
    bars.selectAll("rect")
      ...
      .on('mouseover.tooltip', function(d) {
        tooltip.transition()
          .duration(300)
          .style("opacity", 1);
        tooltip.html(`<strong>${d.name}</strong>`+ " "+d.actual+"")
          .style("left", (d3.event.pageX) + "px")
          .style("top", (d3.event.pageY + 10) + "px");
      })
      .on("mouseout.tooltip", function() {
...
function insert_table (data, id, season) {
    ...
    if (data[0].position_short == "FWD" || "MID") {
    	...
    	for (var i=0; i<p1_5.length; i++){
	    	p1_5[i].was_home == "True" ? turf = "(H)" : turf = "(A)"
	    	last5.push({"GW":Number(p1_5[i].round),"against":`${p1_5[i].opponent_short} ${turf}`,"pts": Number(p1_5[i].total_points), "CS":Number(p1_5[i].clean_sheets),"GC":Number(p1_5[i].goals_conceded),"S":Number(p1_5[i].saves),"B":Number(p1_5[i].bonus)})
	    }
    	var titles = ["GW","against","pts","CS","GC","S","B"];
    	var titles_season = ["against","total_points","clean_sheets","goals_conceded","saves","bonus"];
    } if (data[0].position_short == "DEF") {
    	last5 = []
    	for (var i=0; i<p1_5.length; i++){
	    	p1_5[i].was_home == "True" ? turf = "(H)" : turf = "(A)"
	    	last5.push({"GW":Number(p1_5[i].round),"against":`${p1_5[i].opponent_short} ${turf}`,"pts": Number(p1_5[i].total_points), "CS":Number(p1_5[i].clean_sheets),"GC":Number(p1_5[i].goals_conceded),"YC":Number(p1_5[i].yellow_cards),"A":Number(p1_5[i].assists)})
	    }
    	var titles = ["GW","against","pts","CS","GC","YC","A"];
	...
	if (data[0].position_short == "FWD" || "MID") {
	 ...
	 //    totals_season.pts = totals_season.total_points;
		...
		totals_season.GC = totals_season.goals_conceded;
